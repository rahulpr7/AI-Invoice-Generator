...
aiInvoiceRouter.post('/generate', async (req, res) => {
  try {
    if(!API_KEY){
      return res.status(500).json({ message: "API key not configured" , success: false });
    }
    const { prompt } = req.body;
    if (!prompt || typeof prompt !== "string" || !prompt.trim()) {
      return res.status(400).json({ message: "Invalid prompt", success: false });
    }
    ...
    try {
      ...
      console.error("Failed to parse AI-generated JSON:", {
